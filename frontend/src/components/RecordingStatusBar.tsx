...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-3 py-2 bg-[#F2F5FC] border border-[#DADCE0] rounded-lg mb-2"
    >
      <div className={`w-2 h-2 rounded-full ${isPaused ? 'bg-[#F3C57A]' : 'bg-[#D77A85] animate-pulse'}`} />
      <span className={`text-sm ${isPaused ? 'text-[#171B48]' : 'text-[#171B48]'}`}>
