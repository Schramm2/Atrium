...
impl<R: Runtime> NotificationManager<R> {
    ...
    pub async fn show_notification(&self, notification: Notification) -> Result<()> {
        ...
        let sound_enabled = self.settings.read().await.notification_sound;
        let play_sound = notification.sound && sound_enabled;
        let notification = notification.with_sound(play_sound);
    ...
    /// Get notification statistics for diagnostics.
