...
impl WhisperEngine {
    ...
    pub async fn transcribe_audio_with_confidence(&self, audio_data: Vec<f32>, language: Option<String>) -> Result<(String, f32, bool)> {
        self.transcribe_audio_with_confidence_with_prompt(audio_data, language, None)
            .await
    }

    /// Transcribe audio with an optional decoding prompt for names and other custom terms.
    pub async fn transcribe_audio_with_confidence_with_prompt(
        &self,
        audio_data: Vec<f32>,
        ...
        if let Some(initial_prompt) = initial_prompt.filter(|prompt| !prompt.is_empty()) {
