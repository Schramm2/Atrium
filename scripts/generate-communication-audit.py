...
REWRITES = [
    ("macos/Sources/Notive/Views/AskView.swift", "user", "Ask interface", "Ask view and result states", "Find answers grounded in cited meeting transcript evidence. | Local retrieval | evidence.id | answer.provider · answer.model", "Technical terms and internal identifiers crossed into the narrative chamber.", "Find answers from your meeting transcripts, with citations. | Searches this Mac | no internal ID or provider/model attribution"),
    ("macos/Sources/Notive/Views/AskView.swift", "user", "Ask empty and progress states", "Ask search has no result or is active", "No supporting evidence | Generating an evidence-bound answer", "Technical language did not give the clearest next step.", "Not enough information found | Writing an answer from your transcripts…"),
    ("macos/Sources/Notive/Views/OnboardingView.swift", "user", "onboarding", "onboarding pages", "Local meeting database | Explicit external evidence sharing | recall cited evidence | retrieval", "Database, retrieval, and evidence are internal terms.", "Meeting data stays on this Mac | You choose when transcript excerpts leave this Mac | get cited answers | searches"),
    ("macos/Sources/Notive/Views/HomeView.swift", "user", "home", "home opens", "Capture, review, recall, and write | Recall cited evidence | retrieval | External AI is always explicit", "Abstract or technical language obscured the actual safeguard.", "Record, review, ask, and dictate | Get cited answers | searches | Notive asks before sending anything outside this Mac"),
    ("macos/Sources/Notive/Views/SidebarView.swift", "user", "sidebar", "sidebar opens", "Local | Local-first workspace | Search meeting content | Native Swift · v<version>", "Product and implementation jargon crossed into the narrative chamber.", "On this Mac | Private workspace on this Mac | Search meetings | Version <version>"),
    ("macos/Sources/Notive/Views/MeetingDetailView.swift", "user", "meeting detail", "delete or transcription controls", "speaker aliases from the local database | Retranscribe | transcript segments", "Database and implementation terms were visible.", "speaker names from Notive | Transcribe again | transcript excerpts"),
    ("macos/Sources/Notive/Views/SettingsView.swift", "user", "settings", "General, Appearance, Transcription, Summary, About", "System errors | Test notification sent | Database | canonical dark aubergine workspace | Apple on-device speech | Native Swift", "Technical, misleading, or design-system language appeared in normal settings.", "Problems that need attention | Test notification queued | Meeting data | dark purple theme | On this Mac | removed implementation language"),
    ("macos/Sources/Notive/Support/UpdaterService.swift", "user", "update status", "check or install update", "authenticated GitHub CLI | Checking GitHub Releases | Schramm2/notive | disk image | Notive.app", "Update implementation and repository details leaked into user copy.", "Plain update status with an internet or retry next step"),
    ("macos/Sources/NotiveCore/Stores/AppStore.swift", "user", "error banners and notifications", "record, import, transcribe, summarize, play, or Ask fails", "error.localizedDescription and duplicated notification bodies", "Raw framework, database, provider, path, or identifier detail could reach people; notification bodies repeated headings.", "Operation-specific recovery copy; private raw cause in structured diagnostic logs; non-duplicated notification body"),
    ("macos/Sources/NotiveCore/Services/LanguageProviderService.swift", "user", "AI service error", "remote generation fails", "HTTP <code>: <raw response body>", "Raw transport and response content could leak identifiers or sensitive provider detail.", "The AI service could not complete the request. Check its settings and try again."),
    ("macos/Sources/NotiveCore/Services/ProviderModelService.swift", "user", "model settings error", "model request fails", "HTTP <code>: <raw response body>", "Raw transport and response content crossed into settings.", "The AI service could not load models. Check its settings and try again."),
    ("macos/Sources/Notive/App/NotiveApp.swift", "user", "startup error", "local store initialization fails", "error.localizedDescription", "Raw database or framework detail could appear at startup.", "Notive could not open its local data. Quit Notive, then open it again."),
]
...
DEVELOPER_FINDINGS = [
    ...
    ("script/build_and_run.sh:121-128", "--logs and --telemetry", "Apple unified-log stream", "The viewer is valid, but application coverage depends on explicit Notive events.", "Keep viewer and emit privacy-safe Notive operation events.", "verified"),
    ...
    ("macos/Sources/NotiveCore/Services/GitHubReleaseUpdater.swift:93-170", "update command or rollback fails", "Captured command reason is discarded by the UI mapper", "No operation, exit code, error type, version, or rollback result is recorded.", "update_download|mount|stage|backup|install|verify with outcome, error_type, safe cause, target version, and rollback outcome", "blocked_missing_instrumentation"),
    ...
    ("macos/Sources/NotiveCore/Stores/AppStore.swift", "Ask, recording, import, transcription, summary, playback, or local-data failure", "Structured operation/outcome/error_type/cause/context events", "None after rewrite; cause and context use private privacy annotations.", "unchanged", "verified_source"),
]
...
def main() -> None:
        ...
        for number, line in enumerate(path.read_text().splitlines(), 1):
            if is_shell:
                ...
                broad_ui_scope = relative.startswith((
                    "macos/Sources/Notive/Views/",
                    "macos/Sources/Notive/App/",
            ...
            for value in literals(line):
                ...
                if relative.endswith(("HomeView.swift", "SidebarView.swift")):
                    ...
                    "Ask Notive", "Find answers from your meeting transcripts, with citations.",
                    "Searches this Mac", "What decisions did we make about the launch?",
                    "All meetings", "Limit by date", "Ask across your meeting history",
                    "Notive searches your transcripts and cites the source for each part of the answer.",
    ...
    log_path = "macos/Sources/NotiveCore/Support/DiagnosticLogger.swift"
    ...
    add(rows, location="macos/Sources/NotiveCore/Services/LocalIntelligenceService.swift:85", chamber="user", surface_type="Ask metadata", trigger="answer API called directly with empty evidence", current_copy="local · evidence-only", defect="No current AppStore route reaches this rendered state.", replacement="Generated from meeting evidence", status="blocked_unreachable", verification_result="AppStore stops at insufficient evidence before answer generation; not marked verified.")
