...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "com.ubundi.meet"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "com.ubundi.meet"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "com.ubundi.meet"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "com.ubundi.meet"
