---
version: alpha
name: AuthKit — Atrium Theme Source Reference
description: Reference contract for the Atrium theme, adapted from the AuthKit midnight frosted-glass marketing style. Approved scope is the AuthKit colors, text ramp, and hairline-glass border language only, ported into the existing Atrium geometry, typeface, and spacing. AuthKit fonts, radii, gradients, shadows, and marketing layout are documented below as source evidence, not adopted.
omitted:
  - typography
  - rounded
  - spacing
...
**Theme:** dark (source style). The Atrium adaptation is dark-only and ignores the macOS appearance, like First Motive.

## Overview

**Status — updated for the approved Atrium rebrand.** This file is the **source reference** for the third brand theme, Atrium, added in [docs/rebrand/atrium-rebrand-plan.md](atrium-rebrand-plan.md). The AuthKit marketing style documented below is evidence, recorded faithfully; only the subset under [Atrium Adaptation](#atrium-adaptation-approved-scope) ports into the product. The approved scope is **colors + glass borders only**: the AuthKit palette, text ramp, and hairline-border language adapt to Atrium's existing geometry, typeface, and spacing. Pill buttons, custom fonts, marketing layout, gradients, and shadow-based elevation are out of scope.
...
| --- | --- |
| `docs/rebrand/atrium-rebrand-plan.md` | Approved adaptation scope — colors + glass borders only; the role mapping below; dark-only; Atrium becomes the default theme |
...
The approved plan ports these AuthKit values into the Atrium theme. Front-matter values are the approved colors; the wash and hairline values are composited over `#05060F` using the surface-wash idiom already established in `DESIGN.md`, so the tokens read and contrast-check directly. Plain `primary` / `on-primary` are the default-theme aliases (Atrium becomes the default), matching the existing convention of unsuffixed default tokens.

| AuthKit source | Atrium role | Value | Note |
| --- | --- | --- | --- |
| Void Violet CTA `#663AF3` | accent — primary action | `#663AF3` | White on it ≈ 6.1:1 (AA). The one chromatic action color; AuthKit's single-accent discipline carries over |
| White on Void Violet | on-accent | `#FFFFFF` | |
| Blueprint Blue `#B6D9FC` | secondary accent — info, links, local scope | `#B6D9FC` | |
| Ember Glow `#E46D4C` | warning | `#E46D4C` | AuthKit's demo-showcase orange becomes a functional state color |
| Deep Teal `#269684` | success | `#269684` | AuthKit's customization-swatch teal becomes a functional state color |
| Midnight Canvas `#05060F` | canvas | `#05060F` | Dark-only, ignores macOS appearance like First Motive |
| Frosted-glass wash `#BAD6F7` at 5% over canvas | surface | `#0E101B` (composited) | Wash idiom per `DESIGN.md` |
| Frosted-glass wash `#BAD6F7` at 9% over canvas | raised surface | `#151924` (composited) | |
| Glass Edge hairline `#BAD7F7` at 12% over canvas | border | `#1B1F2B` (composited) | The low-opacity hairline border IS the glass language at this scale; it replaces AuthKit's inset-glow shadow stacks |
| Frost Glow `#D1E4FA` | text | `#D1E4FA` | ≈ 15.6:1 on canvas |
| Fog Veil `#9DA7BA` | text-secondary | `#9DA7BA` | ≈ 8.3:1 on canvas |

**Text ramp.** The ramp Ice Highlight `#D8ECF8` → Frost Glow `#D1E4FA` → Moon Mist `#C7D3EA` → Fog Veil `#9DA7BA` ports as hierarchy only, folded into the existing text / text-secondary roles. The plan adds no new `BrandPalette` roles, so Ice Highlight and Moon Mist are ramp reference steps, not separate tokens.

**Not adopted — explicitly out of scope per the approved plan:**

- The three AuthKit typefaces (Untitled Sans, aeonikPro, dotDigital). Atrium keeps the macOS system family, consistent with the earlier Manrope decision.
- Geometry forks: pill buttons (999px), icon containers (9999px), 16px cards. Atrium keeps its 8/12-point radii and the capsule.
...
## AuthKit Source — Do's and Don'ts
...
## AuthKit Source — Agent Prompt Guide
...
## AuthKit Source — Gradient System
...
## AuthKit Source — Quick Start

**Reference only.** The CSS and Tailwind blocks below reproduce the AuthKit marketing style faithfully for traceability. Atrium adopts only the palette and border language in [Atrium Adaptation](#atrium-adaptation-approved-scope) — do not copy fonts, radii, spacing, or shadows from here into the product.
