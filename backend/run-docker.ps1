...
$AppProjectName = "ubundi-meet-backend"
$AppContainerName = "ubundi-meet-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "ubundi-meet-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server ubundi-meet-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server ubundi-meet-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "ubundi-meet-backend"
        "backend" = "ubundi-meet-backend"
        "meeting" = "ubundi-meet-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "ubundi-meet-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, ubundi-meet-backend"
        ...
        $logArgs += @("whisper-server", "ubundi-meet-backend")
    }
