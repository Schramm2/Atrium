...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.ubundi-meet\meeting_minutes.db",
        "$env:USERPROFILE\Documents\ubundi-meet\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/ubundi-meet-backend/*/backend/meeting_minutes.db"
    }
