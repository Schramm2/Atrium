...
struct RecordingSupportTests {
    ...
    func audioLookup() throws {
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("notive-audio-test-\(UUID().uuidString)", isDirectory: true)
    ...
    func recordingPreferenceMigration() throws {
        let suiteName = "notive-recording-preferences-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let support = FileManager.default.temporaryDirectory
            .appendingPathComponent("notive-preferences-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: support, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: support) }
        let data = try JSONSerialization.data(withJSONObject: [
            "preferences": [
                "save_folder": "/tmp/legacy-notive-recordings",
                "auto_save": false,
            ],
        ])
        ...
        #expect(defaults.string(forKey: RecordingPreferenceStore.folderKey) == "/tmp/legacy-notive-recordings")
        #expect(defaults.bool(forKey: RecordingPreferenceStore.savesAudioKey) == false)

        defaults.set("/tmp/native-notive-recordings", forKey: RecordingPreferenceStore.folderKey)
        defaults.set(true, forKey: RecordingPreferenceStore.savesAudioKey)
        try RecordingPreferenceStore.migrateLegacyPreferences(from: support, defaults: defaults)
        #expect(defaults.string(forKey: RecordingPreferenceStore.folderKey) == "/tmp/native-notive-recordings")
    ...
    func recordingAudioCleanup() throws {
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("notive-cleanup-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: folder) }
    ...
    func importCancellationCleanup() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("notive-import-cancel-test-\(UUID().uuidString)", isDirectory: true)
        ...
        let store = try AppStore(
            databaseURL: root.appendingPathComponent("notive.db"),
    ...
    func importSuccess() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("notive-import-test-\(UUID().uuidString)", isDirectory: true)
        let recordings = root.appendingPathComponent("recordings", isDirectory: true)
        try FileManager.default.createDirectory(at: recordings, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let source = root.appendingPathComponent("synthetic.aiff")
