...
public struct GitHubReleaseUpdater: Sendable {
    ...
    public static let appName = "Notive.app"
    public static let appTarget = "/Applications/Notive.app"
    ...
    public func install(version: String) throws {
        ...
        let temporaryDirectory = fileManager.temporaryDirectory
            .appending(path: "notive-update-\(version)-\(UUID().uuidString)")
        let diskImage = temporaryDirectory.appending(path: "Notive-\(version)-arm64.dmg")
        let mountDirectory = temporaryDirectory.appending(path: "mount")
        let installIdentifier = UUID().uuidString
        let stagedApp = "/Applications/.Notive.app.installing.\(installIdentifier)"
        let backupApp = "/Applications/.Notive.app.backup.\(installIdentifier)"
