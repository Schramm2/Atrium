...
/// Meeting data left on this Mac by an earlier Atrium installation.
...
/// Finds meeting data from an earlier Atrium installation and copies it into the current one.
///
/// Removing Atrium leaves its meeting data in Application Support, so a new installation can
/// take that data back. Atrium stored the data under the bundle identifier until it moved to
/// the `Atrium` folder, and the earlier folder is the first place to look.
public enum PreviousInstallationService {
    ...
    /// Copies the meetings Atrium does not hold yet, and brings their recordings with them.
