...
/// What another Atrium database holds, read before anything is copied from it.
...
public final class SQLiteDatabase: @unchecked Sendable {
    ...
    /// Atrium stored data under the bundle identifier before the `Notive` folder.
    ...
    public init(url: URL, createIfMissing: Bool = true) throws {
        ...
        guard sqlite3_open_v2(url.path, &database, flags, nil) == SQLITE_OK,
            ...
            throw DatabaseError.open("Could not open Atrium database: \(message)")
        }
        connection = database
    ...
    /// Counts the meeting data in another Atrium database without changing it.
    ///
    /// Returns `nil` when the file is missing, is not a Atrium database, or holds no meetings.
    ...
    /// Copies meetings that this database does not hold yet from another Atrium database.
    ///
    ...
    public func importMeetings(from sourceURL: URL) throws -> MeetingImportResult {
        ...
        guard FileManager.default.fileExists(atPath: sourceURL.path) else {
            throw DatabaseError.invalidData("There is no Atrium database at \(sourceURL.path).")
        }
        ...
        guard hasTable("meetings", schema: "previous") else {
            throw DatabaseError.invalidData("That file is not a Atrium meeting database.")
        }
    ...
    /// Counts the meetings in another Atrium database that this database does not hold yet.
    ///
    ...
    /// Reports whether another Atrium database holds a meeting this database does not hold yet.
    ...
    ///
    /// A database written by an earlier Atrium release can be missing a table or a column, so
