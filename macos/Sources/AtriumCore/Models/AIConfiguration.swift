    ...
    static func write(_ value: String, account: String) throws {
        ...
        guard !value.isEmpty else {
            ...
            guard status == errSecSuccess || status == errSecItemNotFound else {
                throw DatabaseError.invalidData("Atrium could not remove the API key from Keychain.")
            }
            return
        }
        ...
        guard updateStatus == errSecItemNotFound else {
            throw DatabaseError.invalidData("Atrium could not save the API key in Keychain.")
        }
        var item = identity
