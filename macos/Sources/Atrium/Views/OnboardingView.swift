...
struct OnboardingView: View {
    ...
    private var accessStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            OnboardingLead(
                "Atrium asks for access only when a feature needs it. Grant what you like now — everything can wait until later in Settings."
            )
            VStack(spacing: 12) {
                PermissionCard(
                    icon: "mic",
    ...
    private var earlierDataStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            OnboardingTitle("Atrium found meetings from before")
            OnboardingLead(
                "An earlier Atrium installation left meeting data on this Mac. Take it back and your meetings, transcripts, notes, summaries, and speaker names return with their recordings."
            )
            EarlierDataCard(
    ...
    private var readyStep: some View {
        VStack(spacing: 16) {
            ...
            Text("Recordings, transcripts, notes, and searches stay on this Mac. Atrium asks before anything leaves it.")
                .font(.title3)
                .foregroundStyle(.secondary)
    ...
    private var footer: some View {
        HStack(spacing: 16) {
            ...
            Button(step == .ready ? "Start Using Atrium" : "Continue") {
                advance()
            }
...
private enum OnboardingStep: CaseIterable, Hashable {
    ...
    var caption: String {
        ...
        case .workspace: "what Atrium does"
...
private struct EarlierDataCard: View {
    ...
    private var detail: String {
        ...
        guard let installation else {
            return "Atrium found no meeting data from an earlier installation."
        }
