...
struct UpdateBanner: View {
    ...
    private var title: String {
        guard let version = updater.updateNoticeVersion else { return "Atrium update" }
        return "Atrium \(version) is available"
    }
    ...
    private var action: some View {
        if case .installing = updater.phase {
            ProgressView()
                .controlSize(.small)
                .accessibilityLabel("Installing Atrium update")
            ...
            .accessibilityLabel("Update Atrium to version \(version)")
            .help(updater.installationBlockReason ?? "Install Atrium \(version)")
        }
    }
}
