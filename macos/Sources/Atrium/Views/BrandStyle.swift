...
struct AtriumPageHeader<Trailing: View>: View {
    let title: String
    let detail: String
...
extension AtriumPageHeader where Trailing == EmptyView {
...
struct BrandMarkView: View {
    ...
    var body: some View {
        Group {
            if let image = BrandAssets.image(named: theme.markAssetName) {
                Image(nsImage: image)
                    .resizable()
                    .scaledToFit()
            } else {
...
struct BrandAtmosphere: View {
    ...
    var body: some View {
        switch theme {
        case .firstMotive:
            if let image = BrandAssets.image(named: "first-motive-atmosphere") {
                Image(nsImage: image)
                    .resizable()
                    .scaledToFill()
                    .opacity(0.34)
