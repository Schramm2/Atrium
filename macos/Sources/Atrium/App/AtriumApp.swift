...
struct AtriumApp: App {
    ...
    init() {
        do {
            ...
            _startupError = State(
                initialValue: "Atrium could not open its local data. Quit Atrium, then open it again."
            )
        }
    }

    var body: some Scene {
        WindowGroup("Atrium", id: "workspace") {
            Group {
                if let store {
                    ...
                    ContentUnavailableView(
                        "Atrium could not open",
                        systemImage: "exclamationmark.triangle",
                        description: Text(
                            startupError
                                ?? "Atrium could not open its local data. Quit Atrium, then open it again."
                        )
                    )
                }
            }
        ...
        MenuBarExtra("Atrium", systemImage: "waveform.badge.mic") {
...
private struct MenuBarView: View {
    ...
    var body: some View {
        ...
        Divider()
        Button("Quit Atrium") { NSApp.terminate(nil) }
    }
}
