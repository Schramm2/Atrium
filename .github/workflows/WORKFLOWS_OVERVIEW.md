...
| `release.yml` | Builds, ad-hoc signs, verifies, and publishes the native Swift Apple Silicon release and signed Sparkle feed for a `vX.Y.Z` tag. |
...
`release.yml` runs only for a pushed `vX.Y.Z` tag. The tag must match `macos/version.json` and point to a commit on `main`. The workflow verifies the native bundle, ZIP, DMG, and signed Sparkle appcast before it publishes the draft release. See [Releasing Notive](../../docs/RELEASING.md) before you push a release tag.
